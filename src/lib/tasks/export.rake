...
task export: :environment do
  ...
  File.open(filename, 'w') do |f|
    ...
    Study.all.sort.each do |s|
      f.write([s.id, JSON.parse(s.authors_json).map{|e| [e['fname'], e['lname']].join(" ") }.join(","), s.title, (j = s.journal) ? j.name : '', s.issue, s.volume, s.year, s.doi, s.phenotypes.select{|e| e.obsolete == false}.size, esc(s.comment), esc(s.description), s.categories.map{|e| e.name}.join(","), s.status.name, s.flybase_ref].join("\t") + "\n")
    end
  end
